#!/usr/bin/env node
// scaffold-plan.mjs - generate the omo-plan draft + plan skeleton deterministically.
//
...
// omo-plan artifact is a NO-OP success (it never overwrites your appended todos),
...
// own writes to resolve under .kiro/omo/ (it does not, and cannot, contain other
// shell commands; it only guarantees the mandated generator never escapes
// .kiro/omo). Mirrors the write.allowedPaths boundary already enforced on the
// `prometheus` Kiro agent (.kiro/omo/plans/**, .kiro/omo/drafts/**), as
// defense-in-depth for shell-invoked writes.
...
// Resolve a project-relative path and confine it under .kiro/omo/ - the script's own
...
export function resolveSafeOmoPath(cwd, relPath) {
	...
	if (!/^\.kiro[/\\]omo([/\\]|$)/i.test(rel.replace(/\\/g, "/").replace(/^\.\//, ""))) {
		throw new Error(`refused: omo-plan may only write under .kiro/omo/: ${relPath}`);
	}
	if (!resolved.toLowerCase().endsWith(".md")) {
		throw new Error(`refused: omo-plan may only write .md files: ${relPath}`);
	}
	return resolved;
}
...
async function assertSafeWriteParent(cwd, target) {
	...
	const omoRoot = resolve(cwd, ".kiro", "omo");
	const parent = dirname(target);
	assertContainedPath(workspaceRoot, parent, `refused: path escapes the workspace root: ${target}`);
	assertContainedPath(omoRoot, parent, `refused: omo-plan may only write under .kiro/omo/: ${target}`);
	...
	assertContainedPath(omoReal, parentReal, `refused: omo-plan may only write under .kiro/omo/ through real paths: ${target}`);
}
...
plan_path: .kiro/omo/plans/${slug}.md
plan_sha256: null
pending-action: write .kiro/omo/plans/${slug}.md`;
	return `---
slug: ${slug}
status: drafting
intent: ${intent}
...
- Evidence: .kiro/omo/evidence/${slug}/task-<N>.<ext>

## Execution strategy
### Execution waves
> Kiro executes sequentially - a wave is an ordering group, not concurrency.
> Target 5-8 todos per wave. Fewer than 3 (except the final) means you under-split.

### Dependency matrix
| Todo | Depends on | Blocks | Independent of |
| --- | --- | --- | --- |

## Todos
> Implementation + Test = ONE todo. Never separate.
<!-- APPEND TASK BATCHES BELOW THIS LINE (edit, or shell >>) - never rewrite the headers above. -->
- [ ] 1. <title>
  What to do / Must NOT do: <...>
  Ordering: Wave <N> | Blocked by: <...> | Blocks: <...>
  ...
  QA scenarios (name the exact tool + invocation): happy + failure, Evidence .kiro/omo/evidence/${slug}/task-1.<ext>
  Commit: <Y/N> | <type>(<scope>): <summary>

## Final verification wave
> Runs after ALL todos, one lane at a time. ALL must APPROVE. Surface results and wait for the user's explicit okay before declaring complete.
...
// Resume-safe write: plain re-run on an existing omo-plan artifact is a no-op
...
export async function writeGuarded(cwd, relPath, content, { reset = false, force = false } = {}) {
	...
	if (existing && existing.trim() !== "") {
		if (!reset) {
			...
			throw new Error(`refused: ${relPath} exists and is not a omo-plan artifact (pass --reset to overwrite)`);
		}

...
description: "Remove AI-generated code smells (slop) from branch changes or an explicit file list. Locks behavior with regression tests FIRST, then runs categorized cleanup sequentially, file by file, then verifies with quality gates. Covers 10 slop categories including performance equivalences, excessive complexity (object annotations, if/elif variant chains), and oversized modules (250+ pure LOC with mandatory modular refactoring). MUST USE when the user asks to \"remove slop\", \"clean AI code\", \"deslop\", \"clean up AI-generated code\", \"remove AI slop\", or wants to clean up AI-generated patterns from recent changes. Triggers - \"remove ai slops\", \"clean ai code\", \"deslop\", \"cleanup AI generated\", \"remove AI slop\", \"clean up AI-generated code\", \"strip slop\", \"ai-slop cleanup\"."
---
...
| Typecheck | `code` tool diagnostics on changed files + project type-checker | zero new errors |
...
### Phase 0: Plan with a numbered checklist

Kiro has no todo tool. Post a numbered checklist covering all phases below in your response, and keep it updated as you move through them — mark the current phase explicitly before starting it.
...
### Phase 4: Sequential slop removal, file by file

Kiro subagents are sequential and blocking — there is no background dispatch and no parallel batching. Process the in-scope file list **one file at a time**, in the order set by Phase 3's plan.

For each file, choose one of two execution modes:

- **Do it yourself** (default, and required when the file is large, high-risk, or touches shared seams): apply the deletion ladder and the category evaluation directly, in this session, using the `write`/`code` tools.
- **Delegate a single bounded file to `sisyphus-junior`** (optional, for small/low-risk files only, to keep this session's context lean): spawn `sisyphus-junior` as a Kiro subagent with a self-contained prompt — it starts fresh with no memory of this skill, so the prompt must restate everything it needs inline (below).

Either way, work through files strictly one after another: finish a file's cleanup, review its diff, and only then move to the next. Never fan out multiple files at once.

**Per-file instructions** (apply directly, or paste into the `sisyphus-junior` prompt when delegating):
...
Then evaluate EVERY one of these 10 categories, applying their KEEP and REFACTOR rules verbatim:
[paste the full "Categories (what counts as slop)" section from this skill's SKILL.md — it is canonical, do not work from a restated subset]
...
**Failure handling**: if a delegated `sisyphus-junior` pass returns without a usable diff, or reports it got blocked, retry that one file once — either re-delegate with a sharper prompt, or fall back to doing it yourself. If the retry also fails, skip that file and escalate it under "Issues Found & Fixed" in the final report; do not let one stuck file stop progress through the rest of the list.
...
4. If genuine slop remains after revert, edit the file directly yourself — one file at a time, sequentially — applying only the changes you can prove are safe.
...
Quality Gates:
  ...
  - Lint: PASS
  - Typecheck (code tool diagnostics + project): PASS (0 new errors on changed files)
...
- If correctness depends on further inspection, keep using the `code` tool's diagnostics, the test runner, and direct file reads until the result is grounded.

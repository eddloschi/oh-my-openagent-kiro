...
|---|---|
| **CLI tool** | Drive `tmux` via the `shell` tool, run the actual command end-to-end, capture output. Paste the session transcript into the journal. Include exit code, stdout, stderr, side-effect check (files created/modified). **`tmux capture-pane` is banned as an evidence source** — it does not preserve cursor movement, colors, or redraw behavior faithfully. For any TUI where the visual rendering itself is the evidence (curses UI, progress bars, colored output, layout), route through `/omo-visual-qa` instead, which renders the pty through a real terminal emulator. |

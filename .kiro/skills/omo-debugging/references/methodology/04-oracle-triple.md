...
Kiro subagents are sequential and blocking — there is no background/parallel dispatch. Spawn all three **one after another**, in order A → B → C, each waiting for the previous to finish. This costs more wall-clock time than upstream's parallel three-Oracle dispatch; budget for it (three sequential Oracle consultations rather than one round-trip) rather than skipping framings to save time.

---

## The three prompts

Spawn `oracle` as a Kiro subagent with Framing A first, wait for it to return, then spawn it again with Framing B, wait, then spawn it again with Framing C:
...
Pass 2 — spawn `oracle` (after Pass 1 returns):
...
Pass 3 — spawn `oracle` (after Pass 2 returns):
"[CONTEXT: bug description + evidence captured so far]
...
Reset the "consecutive failed rounds" counter to 0. Return to Phase 3 (sequential investigation) with the new set.

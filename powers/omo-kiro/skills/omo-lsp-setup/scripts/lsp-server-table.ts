...
// Kiro has no per-project LSP config dotfile to inspect — `/code init` wires up
// servers internally, and `/code status` reports what is currently active.
// This list is intentionally empty in the Kiro port; detect-lsp.ts treats an
// empty list as "config-file detection not applicable, use `/code status`".
export const PROJECT_CONFIG_FILES: readonly string[] = [] as const

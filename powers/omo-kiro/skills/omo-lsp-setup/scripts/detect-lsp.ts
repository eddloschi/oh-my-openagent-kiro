...
// Under Kiro, config wiring is done via `/code init` rather than an inspectable
// per-project dotfile — PROJECT_CONFIG_FILES is empty, and config detection is
// skipped accordingly (see lsp-server-table.ts).
...
function renderReport(root: string, results: readonly DetectionResult[], configState: readonly ConfigFileState[]): string {
	...
	const configSummary =
		configState.length > 0
			? configState
					...
					.join(", ")
			: "not applicable under Kiro — run `/code status` to see wired-up servers"
	...
	lines.push(
		...
		"Next: read references/<language>/README.md to install the missing server, then run `/code init -f`.",
	)
	return lines.join("\n")
}

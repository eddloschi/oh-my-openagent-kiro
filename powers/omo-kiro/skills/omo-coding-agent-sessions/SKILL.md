...
3. **Use explorer-style query lanes when one query batch is not enough.**

If scope is broad (multi-month, many repos/platforms, or a vague "what happened with X"), split independent searches by names/errors, repos/cwds, platforms/models, and time windows, and run those lanes one at a time — either as sequential finder calls, or by spawning a subagent per lane if delegation tools exist (Kiro subagents run sequentially, so this takes longer than the upstream parallel version). Merge candidates by `id`/`path`, then read the most likely sessions.

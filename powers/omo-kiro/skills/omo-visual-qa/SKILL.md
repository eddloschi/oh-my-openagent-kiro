---
name: omo-visual-qa
description: "MUST USE after building/changing any UI or when asked whether a page, component, or TUI looks right. Rigorous visual QA across web/page and terminal UIs. For web capture, probes for `agent-browser` then `playwright` on the machine and uses whichever exists; if neither exists, degrades to code-review-only and says so explicitly. Captures screenshot/TUI evidence with bundled diff scripts, runs design-system/functional and visual-fidelity/CJK reviewer passes (sequential, not parallel — Kiro subagents are sequential and blocking), then synthesizes a good/bad verdict. Triggers: visual QA, screenshot/pixel diff, UI looks wrong, reference fidelity, design system check, responsive check, CJK text clipping, TUI alignment, box-drawing drift."
---

# Visual QA - Dual-Oracle Web and TUI Verification

Verify a rendered UI against intent using objective script evidence plus two sequential read-only oracle passes, then synthesize one good/bad verdict. The script numbers focus the reviewers. They are not the verdict.
...
- Skip when there is no rendered surface (pure backend or library logic with no visual or terminal output). For broad post-implementation review use `/omo-review-work`; this skill is the visual specialist.
...
2. Capture the ACTUAL rendered screenshot at the same viewport size — see **Capture Fallback** below for how to pick the capture tool. Save as PNG.
...
#### Capture Fallback (Kiro has no built-in browser-control tool)

Kiro does not ship a browser-control tool, so before capturing a web screenshot, probe for a capture tool via the `shell` tool, in this order, and use whichever is found first:

1. `command -v agent-browser` — if present, capture with it. See `$SKILL_DIR/references/agent-browser-setup.md` for the full setup, including how to shoot a fixed-viewport screenshot. If it is not installed, you may install it yourself: `npm install -g agent-browser && agent-browser install`.
2. `npx playwright --version` — if that resolves (Playwright is available in the project or globally), drive it via `shell` for navigation and screenshot capture.
3. If NEITHER is available, do **not** silently skip the capture step. State explicitly: "visual capture unavailable — neither agent-browser nor playwright is installed" and degrade to **code-review-only**: run Pass A and Pass B (Step 3 below) against the source code, styles, and markup alone, with no screenshots. Mark every capture-dependent check (pixel diff, hotspot inspection, CJK line-break inspection on rendered text) as `[evidence] SKIPPED — no capture tool available` rather than guessing a verdict from source reading alone.

### TUI

1. Render the TUI through the REAL xterm.js web terminal and screenshot it -
   NEVER use `tmux capture-pane` as evidence, which degrades truecolor
   and misaligns wide (CJK) glyphs. Run the command in a real pty and capture the
   browser render from the repository root:

```
node script/qa/web-terminal-visual-qa.mjs --title "TUI Visual QA" \
  --command "<tui-command>" \
  --input "{ArrowDown}" --input "{Enter}" \
  --evidence-dir .kiro/omo/evidence/<slug>/tui-web-terminal
```
...
```
node "$SKILL_DIR/scripts/visual-qa.mjs" tui-check .kiro/omo/evidence/<slug>/tui-web-terminal/terminal.txt --cols <N>
```
...
## Step 3 - Dispatch two read-only QA subagents, sequentially
...
Spawn `oracle` as a Kiro subagent for each pass. Kiro subagents are sequential and blocking — there is no parallel/background dispatch — so run Pass A to completion first, then run Pass B. Each oracle is read-only: it reviews and reports, it cannot modify files. Each returns PASS, REVISE, or FAIL with concrete, located findings. Pass A proves the surface is a real design-system implementation, not a mock-only or faked-image substitute. Pass B directly opens screenshots and inspects source/content for visual and CJK defects.
...
Spawn `oracle` as a Kiro subagent with this prompt:
...
Wait for Pass A to finish, then spawn `oracle` as a Kiro subagent again with this prompt:
...
2. Code-level design-system fidelity (code oracle). After the pixel-compare pass returns, spawn `oracle` as a Kiro subagent again with this prompt:

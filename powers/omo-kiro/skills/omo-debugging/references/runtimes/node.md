...
```bash
# Long-running inspected process — redirect to a log file so evidence is
# read from the log file, never `tmux capture-pane` (pane scraping is
# banned as an evidence source — see references/methodology/08-qa.md)
tmux new-session -d -s debug-server -c "$PWD"
tmux send-keys -t debug-server 'node --inspect=9229 --import tsx index.ts 2>&1 | tee /tmp/debug-server.log' Enter
...
# Non-blocking inspection from the outside — tail the log file
tail -n 50 /tmp/debug-server.log
```

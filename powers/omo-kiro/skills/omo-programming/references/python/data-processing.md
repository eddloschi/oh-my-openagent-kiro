...
For the deep version (per-operation benchmarks, OOM strategies, full execution templates), load the **`/omo-data-scientist`** skill - it lives in this same skill set and is the source of truth for performance numbers.
...
- Cross-reference - this skill set's `/omo-data-scientist` skill (load it for the deep version)

...
단, R7 조건(WAF 조기 감지)이 성립하면 Claude는 전체 격자를 끝까지 돌리기 전에 먼저 더 저렴한 MCP 정찰 루트로 전환해, 성공하면 그 결과를 채택하고 실패하면 그제서야 원래의 engine 격자로 돌아간다.

**R7 — WAF 조기 감지 시 API-first 전환 분기** (분기 결정은 자동이지만 사용자가 결과에서 확인 가능 — 어떤 우회 경로로 성공/실패했는지 결과 metadata에 명시):
발동 조건 (AND):
...
세 조건 모두 참일 때 Claude는 engine 격자를 잠시 보류하고 **MCP 정찰 경로를 먼저** 순서대로 시도한다:

**Kiro에서의 실행 방식** (Kiro의 Bash 툴과 서브에이전트는 순차 실행만 지원한다 — `run_in_background`에 해당하는 논블로킹 백그라운드 실행이 없으므로, 업스트림의 "engine 백그라운드 + MCP 정찰 동시 진행" 레이스는 그대로 재현할 수 없다):
1. engine 격자를 즉시 돌리지 않고 먼저 MCP 정찰 루트를 실행한다.
2. 정찰에서 내부 API가 식별되면 그 URL로 `python3 -m engine <API_URL>`을 호출해 성공하면 그 결과를 채택하고 끝낸다.
3. 정찰이 실패했거나 쓸만한 API를 찾지 못했다면, 그때 원래의 HTML 격자(`python3 -m engine <URL>`)로 순차 전환한다.
4. 이 순서는 업스트림의 병렬 레이스보다 최선의 경우 느릴 수 있다(정찰이 실패하면 정찰 비용 + 전체 격자 비용을 모두 치른다) — 하지만 대부분의 WAF+SPA 사이트에서는 정찰이 먼저 성공하므로 순차 순서라도 전체 격자보다 훨씬 빠르다.
...
3. 식별된 JSON API URL을 `python3 -m engine <API_URL>`로 재호출. 대부분 API 레이어는 페이지 HTML보다 WAF 보호가 얕아 curl_cffi로 바로 수집됨

...
1. **Concrete visual reference:** the user supplied a reference — treat it as the visual contract, then handle it by kind:
   ...
   Final QA for both runs `/omo-visual-qa` in reference-fidelity mode: compare the actual UI against the reference pixel-by-pixel and verify the code is an extensible design-system implementation, not a screenshot-matched one-off.
2. **Greenfield or fresh setup:** if the user gave no concrete visual reference, design research is a build step with named deliverables — not exploration to be budgeted. Exploration-stop instincts ("enough exploration", two-wave caps) do not apply here. Kiro subagents are sequential and blocking, so run every research lane below one after another (never in parallel) before `DESIGN.md` is written, and open `DESIGN.md` with a `## 0. Research Log` section recording each lane's deliverable — a lane with no Research Log line did not run. Skip a lane only when its tool or network is genuinely unavailable, and name the skip in `DESIGN.md`:
...
> **⚠️ Not vendored in this Kiro port.** Upstream materializes the Layer A taste-skill files (`taste-skill.md`, `gpt-tasteskill.md`, `minimalist-skill.md`, `brutalist-skill.md`, `soft-skill.md`, `redesign-skill.md`, `image-to-code-skill.md`, `output-skill.md`, `stitch-skill.md`, `imagegen-*.md`) and all 70 Layer B brand `DESIGN.md` files from external git submodules (`Leonxlnx/taste-skill` and `nexu-io/open-design` respectively) that were uninitialized in the checkout this port was made from — only the 9 files actually present on disk were copied (`_INDEX.md`, `aside.md`, `clone-from-url.md`, `design-system-architecture.md`, `interaction-skill.md`, `layout-skill.md`, `lazyweb.md`, `react-dev-tooling-skill.md`, `README.md`). If you want the missing taste skills or brand references, clone the relevant repo and drop the files under `.kiro/skills/omo-frontend/references/design/`. Until then, treat `_INDEX.md`'s mood-to-brand mappings as a menu of what *would* be available, fall back to the `aside.md` file that IS present as your one worked Layer B example, and derive taste from `design-system-architecture.md` + the shared axioms below when no taste-skill file exists for the requested mood.
...
> **⚠️ Not vendored in this Kiro port.** Like the Layer B brand files above, `references/ui-ux-db/` (its `README.md`, `data/`, and `scripts/search.py`) is materialized from an external submodule (`nextlevelbuilder/ui-ux-pro-max-skill`) that was uninitialized in the checkout this port was made from, so it does not exist on disk here. If you want the searchable palette/font/UX-guideline database, clone that repo and drop its contents under `.kiro/skills/omo-frontend/references/ui-ux-db/`. Until then, skip this ruleset — fall back to the `design/` ruleset's Layer A/B tokens (and general knowledge) for palette/type/domain questions.
...
`README.md` routes design operating-layer guidance from the pinned `Owl-Listener/designpowers` reference corpus into the existing frontend workflow. Load it — together with `lane-c-review.md` — for every implementation or redesign that creates or updates `DESIGN.md`, and additionally when a task needs explicit personas, accessibility and cognitive constraints, design critique, design debt, handoff, synthetic user testing, motion guidance, or role-reference prompts. It does not replace this frontend skill, `/omo-visual-qa`, `/omo-plan`, `/omo-start-work`, or `/omo-review-work`; it supplies richer design context that must first be distilled into the project `DESIGN.md`, then used as the design-system contract for implementation and verification.

> **Note:** `README.md`, `EVIDENCE.md`, and `UPSTREAM.md` in this ruleset reference a `vendor/` subdirectory (upstream `designpowers` agents/skills materialized from its own pinned submodule). That directory is build output, gitignored upstream, and is not present in this port — the 9 routing/manifest files above are the full designpowers reference set vendored here.
...
| "Build this screenshot / Imagen mock / Stitch output exactly" | `design/README.md` + `design/image-to-code-skill.md` + `perfection/README.md` + `/omo-visual-qa` reference-fidelity mode |
...
- **Done is the `/omo-visual-qa` dual-oracle gate, not your own glance.** A frontend design task is verified through `/omo-visual-qa` (real browser at 375 / 768 / 1280px, every page, with interaction states and motion driven and inspected) until the dual-oracle completion gate passes on fresh evidence.

## When to load something else instead

| Situation | Load |
|---|---|
| Brand/style not among the 70 in `references/design/`, or the user says "Open Design" | `open-design` skill (the local `nexu-io/open-design` library, 137+ design skills, 150+ design systems) if it has been separately vendored into this Kiro install — not ported as part of this batch; see the note above for cloning brand references directly into `references/design/` instead |
| Driving a browser for the Design QA phase | Kiro has no built-in browser-control tool. Probe via `shell`: `command -v agent-browser`, then `npx playwright --version`, and use whichever exists — same Capture Fallback logic as `/omo-visual-qa`. If neither is available, say so and fall back to code/markup review only for the Design QA phase. |
| Pure TypeScript/logic work with zero visual surface | `/omo-programming` skill alone — this skill adds nothing there |

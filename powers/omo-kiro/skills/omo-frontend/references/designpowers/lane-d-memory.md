...
Lane D records the design record around OpenAgent work. It owns `.kiro/omo/frontend-design/state.md`, design debt, handoff notes, retrospective notes, and observational taste memory. It has no hooks and no independent automation path. It records and summarizes what happened so the next OpenAgent phase has reliable context.

## Phase Owner

| Capability | Source boundary | owner | Mapping |
|---|---|---|---|
| Track deferred design and accessibility findings | `design-debt-tracker` | `.kiro/omo/frontend-design/state.md` plus final `/omo-review-work` context | Maintain a register with ID, date, source, severity, issue, affected users, suggested fix, status, and notes. |
| Package design rationale for implementers or reviewers | `design-handoff` | `/omo-start-work` and `/omo-review-work` context | Record component, interaction, accessibility, content, and rationale notes that workers or reviewers need. |
...
| Maintain observational design memory | Authored concept preserved from excluded raw `design-memory` | `.kiro/omo/frontend-design/state.md` and optional personal report context | Store observations descriptively. Do not feed them back as future project constraints. |
...
```text
.kiro/omo/frontend-design/state.md
```
...
Update `.kiro/omo/frontend-design/state.md` with:
...
- `.kiro/omo/frontend-design/state.md` exists before a lane claims durable state.
...
- The evidence index points to real files produced by OpenAgent planning, `/omo-start-work`, `/omo-visual-qa`, or `/omo-review-work`.
...
PASS when state, design debt, handoff, retrospective, and evidence references are current enough for `/omo-plan`, `/omo-start-work`, `/omo-visual-qa`, or `/omo-review-work` to resume without guessing.

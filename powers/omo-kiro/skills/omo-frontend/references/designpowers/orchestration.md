...
Use `.kiro/omo/frontend-design/state.md` as the design operating ledger when the active workflow is allowed to write OpenAgent state. If the current task forbids editing `.omo`, read it as context only and report any needed updates in the handoff.
...
| Verification Matrix | Required frontend design/perfection, `/omo-visual-qa`, persona walkthrough, and `/omo-review-work` evidence. |
...
- For significant implementation work, close through `/omo-review-work`; for visual work, run `/omo-visual-qa` first.
...
- route actual phase ownership to frontend design/perfection, `/omo-plan`, `/omo-start-work`, `/omo-visual-qa`, or `/omo-review-work`;
...
- final `/omo-review-work` verdict when the work was significant enough to require review.

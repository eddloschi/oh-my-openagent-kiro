...
Before declaring the task done, verify the rendered UI. **The verification authority is `/omo-visual-qa`, not a hand-rolled checklist here.** Run `/omo-visual-qa`: it captures every page and breakpoint (375 / 768 / 1280px) on fresh evidence, drives and inspects interaction states (hover/focus/active) and motion (transitions, scroll-triggered, load), runs the dual-oracle pass, and loops until an independent reviewer passes. For a concrete reference or clone, run it in reference-fidelity mode.

This skill adds only the design-taste judgments `/omo-visual-qa` cannot make for you:
...
Report "done" only when `/omo-visual-qa` has passed on fresh evidence AND neither a visual bug nor a floor-level or slop-laden surface remains.
